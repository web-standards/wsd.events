shower.modules.require(['shower.global'], function (sh) {
	sh.ready(function (instance) {

		var slides = instance.getSlides();

		slides
			.filter(function (s) {
				return !!s.layout.getData('diagram');
			})
			.forEach(function (s) {
				s.options.steerage = {
					activate: activate,
	...
	var s = Snap('#transformation-diagram');
	var currentStep = 0;

	var steps = [
		function () {
			renderHeader(s, 140, 30, 'Разбираем', function () {
				renderArc(s, 70, 230, 140, function () {
					renderSourceTree(s, 230, 90);
				});
			});
		},
		function () {
			renderHeader(s, 320, 30, 'Преобразуем', function () {
				renderArc(s, 230, 410, 320, function () {
					renderTransformedTree(s, 410, 90);
				});
			});
		},
		function () {
			renderHeader(s, 500, 30, 'Сериализуем', function () {
				renderArc(s, 410, 570, 500, function () {
					renderTransformedCode(s, 515, 90);
				});
			});
		}
	];
